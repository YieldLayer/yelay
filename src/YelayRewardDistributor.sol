 ...
 */
contract YelayRewardDistributor is Pausable, YelayOwnable, IRewardDistributor {
     ...
     */
    function payRewards(address account, IERC20[] memory tokens, uint256[] memory amounts)
        external
        override
        whenNotPaused
        onlyDistributor
    {
        require(
            tokens.length == amounts.length, "RewardDistributor::payRewards: Tokens should be of same length as amounts"
        );
     ...
     */
    function payReward(address account, IERC20 token, uint256 amount) external override whenNotPaused onlyDistributor {
     ...
     * @param amount amount of reward to send
     */
    function _payReward(address account, IERC20 token, uint256 amount) private {
     ...
     */
    function retrieveRewards(address account, IERC20 token, uint256 amount) external onlyOwner {
        token.safeTransfer(account, amount);
