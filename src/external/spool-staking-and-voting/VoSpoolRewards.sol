...
contract VoSpoolRewards is SpoolOwnable, IVoSpoolRewards {
     ...
     */
    constructor(address _spoolStaking, IVoSPOOL _voSpool, ISpoolOwner _spoolOwner, uint256 _fullPowerTranchesCount)
    ...
    function updateVoSpoolRewardRate(uint8 toTranche, uint112 rewardPerTranche) external onlyOwner {
        ...
        VoSpoolRewardRate memory voSpoolRewardRate = VoSpoolRewardRate(currentTrancheIndex, toTranche, rewardPerTranche);
        ...
        emit RewardRateUpdated(
            voSpoolRewardRate.fromTranche, voSpoolRewardRate.toTranche, voSpoolRewardRate.rewardPerTranche
        );
    }
    ...
    function endVoSpoolReward() external onlyOwner {
        ...
        require(
            currentRewardRate.toTranche > currentTrancheIndex, "VoSpoolRewards::endVoSpoolReward: Rewards already ended"
        );
    ...
    function _storeVoSpoolForNewIndexes() private {
        ...
        IVoSPOOL.GlobalGradual memory global = voSpool.getNotUpdatedGlobalGradual();
    ...
    function _updateUserVoSpoolRewards(address user) private {
        IVoSPOOL.UserGradual memory userGradual = voSpool.getNotUpdatedUserGradual(user);
        ...
        if (trancheIndex < lastFinishedTrancheIndex) {
            ...
            VoSpoolRewardRate[] memory voSpoolRewardRatesArray =
                _getRewardRatesForIndex(voSpoolRewardUser.lastRewardRateIndex);
