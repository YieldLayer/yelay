...
contract SYLAYTest is Test, Utilities {
    ...
    function contractDeployment() public {
        ...
        yelayMigrator = new YelayMigrator(
            spoolOwner,
            yLAY,
            sYLAY,
            IYelayStaking(address(yelayStaking)),
            SPOOL
        );
        assert(address(yelayMigrator) == precomputedMigratorAddress);

        yLAY.initialize(yelayMigrator);
    ...
    function contractSetup() public {
        ...
        uint _gasUsed = _gasBefore - gasleft();
    ...
    function test_mintGradualPowerOverMultiplePeriods() public setUpGradualVotingPower {
        ...
        uint256 expectedMaturedAmountTotal = expectedMaturedAmount1
            + expectedMaturedAmount2
            + expectedMaturedAmount3
            + expectedMaturedAmount4
            + expectedMaturedAmount5;
    ...
    function test_mintZeroGradualPowerToUser() public setUpGradualVotingPower {
        ...
        UserGradual memory userGradual = sYLAY.getUserGradual(user1);
        ...
        GlobalGradual memory globalGradual = sYLAY.getGlobalGradual();
    ...
    function test_mintAndBurnGradualPower() public setUpGradualVotingPower {
        ...
        uint256 mintAmount = 1000 ether;  // Mint amount in ether
        GlobalGradual memory globalGradual = sYLAY.getGlobalGradual();
        uint totalMaturingAmountBefore = globalGradual.totalMaturingAmount;
        uint totalRawUnmaturedVotingPowerBefore = globalGradual.totalRawUnmaturedVotingPower;
        uint totalMaturedVotingPowerBefore = globalGradual.totalMaturedVotingPower;
        uint lastUpdatedTrancheIndexBefore = globalGradual.lastUpdatedTrancheIndex;
    ...
    function test_burnGradualPowerMultipleTimes() public setUpGradualVotingPower {
        ...
        uint256 expectedMaturedAmount2 = getVotingPowerForTranchesPassed(userAmountAfterBurn, weeksPassed1 + weeksPassed207);
    ...
    function test_transferUserNormal() public {
        ...
        UserGradual memory user1Gradual = sYLAY.getUserGradual(user1);
        ...
        UserGradual memory user1GradualBefore = sYLAY.getUserGradual(user1);
        vm.prank(gradualMinter);
        sYLAY.transferUser(user1, user2);
    
        // ASSERT - Verify user1 has nothing and user2 has the same amounts
        UserGradual memory user1GradualAfter = sYLAY.getNotUpdatedUserGradual(user1);
        ...
        UserGradual memory user2Gradual = sYLAY.getUserGradual(user2);
